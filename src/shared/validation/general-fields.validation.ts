...
// Helper transformer to parse comma-separated strings or arrays into clean string arrays
export const parseCommaSeparatedArray = (val: string | string[]) => {
	if (typeof val === 'string') {
		return val
			.split(',')
			.map((item) => item.trim())
...
export const commaSeparatedArray = z.preprocess(parseCommaSeparatedArray, z.array(z.string().optional()));
