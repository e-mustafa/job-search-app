...
const createFileFilter = (typeInput: TFileType | TFileType[]) => {
	return (_req: Request, file: Express.Multer.File, cb: multer.FileFilterCallback) => {
		...
		// Allow generic octet-stream temporarily to pass to the signature verification layer
		const isGenericOctetStream = file.mimetype === 'application/octet-stream';
		const isValidMime = allowedList.includes(file.mimetype);

		if (isAll || isValidMime || isGenericOctetStream) {
			cb(null, true);
		} else {
...
export type TUploadFactory = {
	...
	type?: TFileType | TFileType[] | undefined;
...
export const uploadCloud = (type?: TFileType | TFileType[], size?: number) =>
	uploadFactory({ storageType: 'cloud', type, size });
