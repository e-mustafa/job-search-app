...
export const deleteMultipleFromCloudinary = async (
	files: (Pick<UploadResult, 'public_id' | 'resourceType'> & { secure_url?: string })[] | UploadResult[],
): Promise<void> => {
...
export const uploadUserProfileMedia = (
	...
	return uploadToCloudinary(file, {
		folder: `job_search/users/${userId}/profile`,
 ...
 */
export const uploadCompanyAttachments = (files: IFile[], companyId: Id, prefix?: string): Promise<UploadResult[]> => {
	return uploadMultipleToCloudinary(files, {
		folder: `job_search/companies/${companyId}`,
		prefix: prefix || 'attachment',
	});
};

export const uploadResumeAttachment = (file: IFile, userId: Id, prefix: string = 'resumes'): Promise<UploadResult> => {
	return uploadToCloudinary(file, {
		folder: `job_search/users/${userId}/resumes`,
		prefix,
	});
};
