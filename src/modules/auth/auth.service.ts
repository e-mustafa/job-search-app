...
const selectUserInfo = 'firstName lastName email mobileNumber username bio gender avatar cover role';
...
class AuthServices {
	 ...
	 */
	async register({ firstName, lastName, email, password, confirmPassword }: IRegisterDTO): Promise<boolean> {
	...
	async verifyAccount(email: string, otp: string | number): Promise<boolean> {
		...
		const user: IUser = await this.userRepo.findByEmail(email).exec();
		if (!user) {
		...
		const update = await this.userRepo.updateOne({ _id: user._id }, { isConfirmed: new Date() });
	...
	async socialLogin_google(provider: TProvider, idToken: string): Promise<{ isNew: boolean; tokens: TTokens }> {
		...
		const newUser: IUserDocument = await this.userRepo.create({
			...
			avatar: picture ? { public_id: 'google-picture', secure_url: picture } : null,
			provider,
		});
